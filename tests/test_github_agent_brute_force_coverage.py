...
def mock_session():
    with patch("requests.Session") as mock_s:
        ...
        response.json.return_value = {"id": 1, "name": "test", "html_url": "http://test"}
        response.text = '{"id": 1}'
...
def test_github_api_brute_force(mock_session):
...
def test_mcp_server_coverage(mock_session):
    ...
    with patch("github_agent.auth.get_client") as mock_gc:
...
def test_auth_delegation(mock_session):

...
accounts (e.g. the `example` user **and** the `Knuckles-Team` org), determines
...
- `github_search action=issues {"q": "user:example is:open is:issue"}`
- `github_search action=issues {"q": "user:example is:open is:pr"}`
